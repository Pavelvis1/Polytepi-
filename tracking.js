...
function beginCheckout() {
  var cart = getCart();
  if (cart.length === 0) { showToast('Заявка пуста — добавьте товары'); return; }
...
function purchase() {
  ...
  updateCartBadge();
  return { txnId: txnId, value: value };
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('[data-checkout]').forEach(function (btn) {
    btn.addEventListener('click', function (e) { e.preventDefault(); beginCheckout(); });
  });
